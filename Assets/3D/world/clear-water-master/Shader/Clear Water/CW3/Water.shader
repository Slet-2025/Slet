...
{	
	#if defined(_USE_FOAM)
	pix.foamMask = getFoam(tc, wind, dirToSurface, vertexFoam) * _FoamColor.a;

	// Apply fadeout to top
	pix.alpha = saturate(pix.alpha + pix.foamMask);

	// Todo: Is there a better way to blend this?
	pix.diffuseColor.rgb = lerp(pix.diffuseColor, _FoamColor.rgb, saturate(pix.foamMask * 3));
	...
	) : SV_TARGET
{
    ...
    if (_Mode == 1.0 ) // Opaque mode
    {
		...
		UnityGI baseGI = UnityGI_Base_local(giInput, bakedAtten, occlusion, 
			pix.worldNormal, pix.perceptualRoughness, float3(0, 0, 0), _ExposureOcclusion, _LightmapSpecularMaxSmoothness,
			_ProbeTransmission, getSSSParams(attenuation));

		UnityIndirect indirectLight = baseGI.indirect;
		...
		float3 col = BRDF_New_PBS(
			...
			occlusion,
			transmission,
			getSSSParams(attenuation),
			light,
			indirectLight,
			float3(0, 0, 0) // No refraction in opaque mode
		);
	...
	float surfaceEdgeFade = saturate(distToSurface); // Edge fade to handle thin layers of water
	pix.alpha = pix.alpha * lerp(1.0, surfaceEdgeFade, _SurfEdgeFade); // Fade out alpha at edges
	...
	float3 refrAbsorption = waterAbsorption * min(waterDepth, _RefractMaxDepth) * absorptionUserTweak; // What colour gets absorbed by the medium. 
	...
	// todo: should this affect edges?  // pix.alpha?
	...
	half occlusion = 1.0; // returned with lightmap occlusion
    UnityGI baseGI = UnityGI_Base_local(giInput, bakedAtten, occlusion, 
		pix.worldNormal, pix.perceptualRoughness, transmission, _ExposureOcclusion, _LightmapSpecularMaxSmoothness,
	...
	*/
	...
	//UNITY_APPLY_FOG(i.fogCoord, col); 
	#ifdef UNITY_PASS_FORWARDADD
	return float4(col * attenuation, 1.0 * attenuation);
	#else
